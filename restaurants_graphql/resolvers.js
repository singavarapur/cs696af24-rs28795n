...
const resolvers = {
  Query: {
    ...
    async restaurants(_, { borough, cuisine }) {  // New resolver for the restaurants query
      let collection = await db.collection("restaurants");
      let query = {};
      
      if (borough) {
        query.borough = borough;  // Filter by borough if provided
      }
      if (cuisine) {
        query.cuisine = cuisine;  // Filter by cuisine if provided
      }
      
      return await collection.find(query).toArray();  // Return all matching restaurants
    }
  },
